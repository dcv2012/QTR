...
def run_mmr_rhc(num_runs: int, kind: str, output_dir: str):
    path = Path(output_dir)
    
    if not path.exists():
        os.makedirs(path)
    file_path = path.joinpath(f"deepmmr_rhc_{kind.lower()}.csv")
    
    with open(Path(str(pathlib.Path(__file__).parent.parent.parent.parent / 'configs' / 'rhc' / f'mmr_rhc_{kind.lower()}_1.json')),'r') as f:
        model_config1 = json.load(f)
    
    with open(Path(str(pathlib.Path(__file__).parent.parent.parent.parent / 'configs' / 'rhc' / f'mmr_rhc_{kind.lower()}_0.json')),'r') as f:
        model_config0 = json.load(f)
    ...
    for i in range(num_runs):
        ...
        trainer1 = MMR_Trainer_RHC(model_config1, random_seed)
        trainer0 = MMR_Trainer_RHC(model_config0, random_seed)
        ...
        res0 = test_data_t.outcome.squeeze(-1) * pred0.squeeze(-1) * (test_data_t.treatment.reshape(-1) == torch.zeros(test_data_t.treatment.shape[0])).float()
        res = [res1.mean().item() , res0.mean().item()]
        # results.append([f'DeepMMR_q_{kind}'] + res)
        
        with open(file_path, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([f'DeepMMR-{kind.upper()}'] + res)
