...
# Split data into 60%:20%:20% partitions.
# 60% for train, 20% for validation/model selection, and 20% for computing the ATE at test time
np.random.seed(RANDOM_SEED)
train, validate, test = np.split(df.sample(frac=1, random_state=RANDOM_SEED), [int(.7*len(df)), int(.85*len(df))])
